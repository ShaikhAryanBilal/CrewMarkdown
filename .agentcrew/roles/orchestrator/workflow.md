...
process:
  - Check SDLC Model — if .agentcrew/state/sdlc-selection.json exists, load it for phase ordering; otherwise use default O1→O2→...→O8
...
## Instructions
1. Check SDLC model — if `.agentcrew/state/sdlc-selection.json` exists, load it for phase ordering, gate sequence, parallelism rules. Otherwise use default linear order (O1→O2→...→O8).
...
8. Log to `.agentcrew/logs/<yyyy>/<MM>/<dd>/<HHmmss>/<chat-slug>-[<HHmmss>]/orchestrator.md` (derive `<chat-slug>` from request text)
...
| SDLC model selection (optional) | .agentcrew/state/sdlc-selection.json | Phase ordering per model | User, Squads |
