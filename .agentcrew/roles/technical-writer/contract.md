...
## Known Procedures
1. Input → technical spec → audience analysis → draft docs → technical review (Dev/Architect) → editorial review → publish
2. API docs: OpenAPI spec → auto-generated reference → manual narrative augmentation → review with Backend → publish
3. Release notes: release scope → draft per-audience sections → review with PM/DevOps → publish alongside release
