...
2. Load `.agentcrew/config/sdlc-models.md` for full model definitions
...
9. Write `.agentcrew/state/sdlc-selection.json` with:
   - selected model
   - rationale
...
11. Log to `.agentcrew/logs/<yyyy>/<MM>/<dd>/<HHmmss>/<chat-slug>-[<HHmmss>]/<role>.md` (derive `<chat-slug>` from request text)
...
| User decision | User | `.agentcrew/state/sdlc-selection.json` | State, All objectives |
...
|-----------------------------------|
| See `.agentcrew/config/sdlc-models.md` for per-model phase map |
