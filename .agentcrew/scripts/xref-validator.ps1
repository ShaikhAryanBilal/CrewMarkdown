<#
.SYNOPSIS
Validates all cross-references: Next→, Revert→, and Markdown links to .md files across .agentcrew/.
.PARAMETER Path
Root path to validate (default: .agentcrew)
.EXAMPLE
.\.agentcrew\scripts\xref-validator.ps1
#>
...
if (-not $Path) { $Path = Join-Path $ProjectRoot ".agentcrew" }
